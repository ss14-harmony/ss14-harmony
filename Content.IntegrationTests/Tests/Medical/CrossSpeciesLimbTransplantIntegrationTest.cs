...
public sealed class CrossSpeciesLimbTransplantIntegrationTest : InteractionTest
{
    ...
    public async Task CrossSpeciesLimbTransplant_FullFlow_Completes()
    {
        ...
        // --- Verify grafted arm is fully attached and patient can hold in both hands ---
        await Server.WaitAssertion(() =>
        {
            var graftedArm = SEntMan.GetEntity(graftedArmNet);
            Assert.That(SEntMan.TryGetComponent(graftedArm, out BodyPartComponent? armBodyPart), Is.True);
            Assert.That(armBodyPart!.Body, Is.EqualTo(human), "Grafted vox arm should be fully attached to human");
            ...
            Assert.That(ev.Parts, Has.Count.EqualTo(1), "Human should have exactly one ArmLeft (the grafted vox arm)");
        });

        await Server.WaitPost(() =>
        {
            Assert.That(SEntMan.TryGetComponent(human, out HandsComponent? humanHands), Is.True, "Patient should have hands");
            Assert.That(HandSys.GetHandCount((human, humanHands!)), Is.GreaterThanOrEqualTo(2), "Patient should have at least 2 hands");

            var item1 = SEntMan.SpawnEntity("Screwdriver", SEntMan.GetCoordinates(TargetCoords));
            var item2 = SEntMan.SpawnEntity("Wrench", SEntMan.GetCoordinates(TargetCoords));

            Assert.That(HandSys.TryPickupAnyHand(human, item1, checkActionBlocker: false), Is.True, "Patient should pick up item in first hand");
            Assert.That(HandSys.TryPickupAnyHand(human, item2, checkActionBlocker: false), Is.True, "Patient should pick up item in second hand");

            var heldCount = HandSys.EnumerateHeld((human, humanHands!)).Count();
            Assert.That(heldCount, Is.EqualTo(2), "Patient should be holding items in both hands");
        });
        await RunTicks(1);
