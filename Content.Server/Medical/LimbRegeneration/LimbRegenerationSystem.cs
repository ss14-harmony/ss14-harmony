...
public sealed class LimbRegenerationSystem : EntitySystem
{
    ...
    public void OnOrganRemovedFrom(Entity<BodyComponent> ent, ref OrganRemovedFromEvent args)
    {
        ...
        if (!TryComp<HumanoidProfileComponent>(ent, out var profile) || profile.Species != SlimePerson)
            return;
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var regen, out var bodyComp, out var profile))
        {
            ...
            for (var i = 0; i < regen.PendingRegenerations.Count; i++)
            {
                ...
                if (RestoreSingleLimb(uid, profile.Species, entry.Category, bodyComp))
                    toRemove.Add(i);
            }
    ...
    public void RestoreAllLimbs(EntityUid body)
    {
        ...
        {
            RestoreSingleLimb(body, profile.Species, category);
        }
    }
}
