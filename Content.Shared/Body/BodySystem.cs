...
public sealed partial class BodySystem : EntitySystem
{
    ...
    [Dependency] private readonly SurgeryLimbTaggingSystem _surgeryLimbTagging = default!;
    ...
    private void OnBodyEntInserted(Entity<BodyComponent> ent, ref EntInsertedIntoContainerMessage args)
    {
        ...
        if (_bodyPartQuery.TryComp(args.Entity, out var bodyPart))
        {
            ...
            // Re-attached limbs keep their SurgeryLayerComponent state (performed steps) from when they were detached.
            // Do not reset - the user must perform the close steps (MaintainAlignment, SealBleedPoints, etc.) to close the layers.
    ...
    private void OnBodyPartQuery(Entity<BodyComponent> ent, ref BodyPartQueryEvent args)
    {
        ...
        // Catch-up: InitialBody species (Moth, Diona, Vox, etc.) have Head/Torso organs without BodyPartComponent.
        // LimbDetachmentEffectsSystem tags them on EntGotInsertedIntoContainerMessage, but ensure they're tagged
        // in case of timing/ordering issues (e.g. state application, late join).
        var body = ent.Owner;
        foreach (var part in ent.Comp.Organs.ContainedEntities)
        {
            if (!_organQuery.TryComp(part, out var organ) || organ.Category is not { } category)
                continue;

            var categoryStr = category.ToString();
            if ((categoryStr is "Torso" or "Head") && !HasComp<BodyPartComponent>(part) && !HasComp<SurgeryLayerComponent>(part))
                _surgeryLimbTagging.TagLimbOrgan(part, body, category);
        }
