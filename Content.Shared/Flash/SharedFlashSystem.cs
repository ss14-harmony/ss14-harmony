...
using Content.Shared.Eye.Blinding.Systems; // Funky - CyberMed
...
public abstract class SharedFlashSystem : EntitySystem
{
        ...
        TimeSpan? stunDuration = null)
    {
        ...
        // Funky - CyberMed: let cyber-eyes (and similar) scale and/or shorten the effective flash duration.
        ...
        if (!_statusEffectsSystem.TryAddStatusEffect<FlashedComponent>(target, FlashedKey, adjustedDuration, true)) // Funky - CyberMed
            return;

        if (stunDuration != null)
            _stun.TryUpdateParalyzeDuration(target, stunDuration.Value);
        else
            _movementMod.TryUpdateMovementSpeedModDuration(target, MovementModStatusSystem.FlashSlowdown, adjustedDuration, slowTo); // Funky - CyberMed
