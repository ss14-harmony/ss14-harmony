...
public sealed class SurgeryLayerSystem : EntitySystem
{
    ...
    /// For grafted limbs (e.g. vox arm on human), uses limb's species so DetachLimb/InsertOrgan show correctly.
    /// </summary>
    public BodyPartSurgeryStepsPrototype? GetStepsConfig(EntityUid body, EntityUid bodyPart)
    {
        if (TryComp<SurgeryBodyPartComponent>(bodyPart, out var surgeryBodyPart))
        {
            if (surgeryBodyPart.StepsConfigId is { } stepsConfigId &&
                ...
                return stepsConfig;
            var config = GetStepsConfig(surgeryBodyPart.SpeciesId, surgeryBodyPart.OrganCategory);
            if (config != null)
                return config;
            // Fallback: limb's species may lack config (e.g. custom species) - use body's species so surgery still works
            if (TryComp<HumanoidProfileComponent>(body, out var bodyProfile))
                return GetStepsConfig(bodyProfile.Species, surgeryBodyPart.OrganCategory);
        }

        if (!TryComp<HumanoidProfileComponent>(body, out var profile) || !TryComp<OrganComponent>(bodyPart, out var organ) || organ.Category is not { } category)
            return null;

        return GetStepsConfig(profile.Species, category);
    }
    ...
    public IReadOnlyList<string> GetAvailableStepsForEmptySlot(EntityUid body, string categoryId)
    {
        ...
        var stepsConfig = GetStepsConfig(profile.Species, category);
        if (stepsConfig == null)
            return Array.Empty<string>();
    ...
    public IReadOnlyList<(string StepId, NetEntity Organ)> GetAvailableOrganSteps(EntityUid body, EntityUid bodyPart)
    {
        ...
        foreach (var organ in bodyPartComp.Organs.ContainedEntities)
        {
            ...
            // Show removal OR insertion procedures, not both. If organ was just inserted (OrganInsertProgress
            // has entry) and insertion steps are incomplete, show only insertion; else show only removal.
            var insertEntry = layerComp.OrganInsertProgress.FirstOrDefault(e => e.Organ == organNet);
            var insertionIncomplete = insertEntry != null && organProcs.InsertionProcedures.Count > 0
                && organProcs.InsertionProcedures.Any(p => !insertEntry.Steps.Contains(p.ToString()));

            if (insertionIncomplete)
            {
                foreach (var proc in organProcs.InsertionProcedures)
                {
