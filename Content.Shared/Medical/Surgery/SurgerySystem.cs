...
public sealed class SurgerySystem : EntitySystem
{
    ...
    private void OnSurgeryRequest(Entity<BodyComponent> ent, ref SurgeryRequestEvent args)
    {
        ...
        if (TryComp<HumanoidProfileComponent>(ent.Owner, out var profile))
        {
            if (profile.Species == (ProtoId<SpeciesPrototype>)"SlimePerson" &&
            ...
            if (profile.Species == (ProtoId<SpeciesPrototype>)"Skeleton" &&
        ...
        if (isAttachLimbToEmptySlot)
        {
            ...
            stepsConfig = _surgeryLayer.GetStepsConfig(humanoidProfile.Species, limbCategory);
        }
        else
        {
        ...
        if (args.ProcedureId == "DetachLimb")
        {
            ...
            // BodyPartComponent (Human): limb has Body and contains hand/foot. InitialBody species: limb is organ in body directly.
            if (TryComp<BodyPartComponent>(args.BodyPart, out var limbBodyPart))
            {
                if (limbBodyPart.Body != ent.Owner)
                {
                    args.RejectReason = "body-part-detached";
                    return;
                }
            }
            else if (limbOrgan.Body != ent.Owner)
            {
                args.RejectReason = "body-part-detached";
                return;
            }
    ...
    private void ApplyOrganStep(Entity<BodyComponent> ent, EntityUid bodyPart, string stepId, NetEntity? organNet, EntityUid? organUid, SurgeryProcedurePrototype? procedure, SurgeryStepPrototype? step, EntityUid user)
    {
        ...
        if (!isAttachLimbToEmptySlot)
        {
            // DetachLimb: limb can be BodyPartComponent (Human) or OrganComponent (InitialBody species)
            var bodyPartValid = TryComp<BodyPartComponent>(bodyPart, out bodyPartComp) && bodyPartComp.Body == ent.Owner
                || (stepId == "DetachLimb" && TryComp<OrganComponent>(bodyPart, out var limbOrgan) && limbOrgan.Body == ent.Owner);
            if (!bodyPartValid)
            {
        ...
        else if (stepId == "AttachLimb")
        {
            ...
            if (ent.Comp.Organs != null && _container.Insert(limb, ent.Comp.Organs))
            {
                ...
                // Grafted limbs retain PerformedOrganSteps from when they were detached (including DetachLimb).
                // Clear DetachLimb so the limb can be amputated again from its new body.
                if (TryComp<SurgeryLayerComponent>(limb, out var limbLayer))
                {
                    limbLayer.PerformedOrganSteps.Remove("DetachLimb");
                    Dirty(limb, limbLayer);
                }
