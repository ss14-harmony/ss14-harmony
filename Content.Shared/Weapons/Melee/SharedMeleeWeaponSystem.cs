...
public abstract class SharedMeleeWeaponSystem : EntitySystem
{
    ...
    public bool TryGetWeapon(EntityUid entity, out EntityUid weaponUid, [NotNullWhen(true)] out MeleeWeaponComponent? melee)
    {
        ...
        if (_hands.TryGetActiveItem(entity, out var held))
        {
            ...
            // Cyber arm virtual item that isn't a melee weapon: suppress punching so the item is used via interaction instead.
    ...
    protected virtual void DoLightAttack(EntityUid user, LightAttackEvent ev, EntityUid meleeUid, MeleeWeaponComponent component, ICommonSession? session)
    {
        ...
        if (Damageable.TryChangeDamage(target.Value, modifiedDamage, out var damageResult, origin:user, ignoreResistances:resistanceBypass, armorPenetration: component.ArmorPenetration)) // Goob: armor penetration
        {
    ...
    private bool DoHeavyAttack(EntityUid user, HeavyAttackEvent ev, EntityUid meleeUid, MeleeWeaponComponent component, ICommonSession? session)
    {
        ...
        for (var i = targets.Count - 1; i >= 0; i--)
        {
            ...
            var damageResult = Damageable.ChangeDamage(entity, modifiedDamage, origin: user, ignoreResistances: resistanceBypass, armorPenetration: component.ArmorPenetration); // Goob: armor penetration
